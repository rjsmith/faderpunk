...
use crate::fp_grids_lib::utils::{Random, u8_mix, u8_u8_mul, u8_u8_mul_shift8};
...
impl PatternGenerator {
    ...
    pub fn tick(&mut self, external_clock_tick: bool) {
        ...
        for part in 0..K_NUM_PARTS {
            self.euclidean_step[part] = (self.euclidean_step[part] + 1) % self.current_euclidean_length[part];
        }

        self.first_beat_ = self.sequence_step_ == 0;
        let mut steps_per_beat = K_NUM_STEPS_PER_PATTERN / 4;
        if steps_per_beat == 0 { steps_per_beat = 1; } // Avoid division by zero for short patterns
    ...
    pub fn set_fill(&mut self, channel:usize, fill_param_value: u8) {
        ...
        self.settings_[OutputMode::OutputModeEuclidean.ordinal() as usize].density[channel] = fill_param_value;
        ...
        let mut active_steps: u8 = ((fill_param_value as u16 * current_length as u16 + 127) / 255) as u8; // Add 127 for rounding
   ...
   fn read_drum_map(&self, step: u8, instrument: u8, x: u8, y: u8) -> u8 {
    ...
    fn evaluate_drums(&mut self) {
        if self.step_ == 0 && self.pulse_ == 0 {
            ...
            let mut randomness = match self.settings_[OutputMode::OutputModeDrums.ordinal() as usize].options {
                PatternModeSettings::Drums { randomness, .. } => randomness,
                _ => 0, // Default to 0 if not in Drum mode, though this should never happen
            };
            randomness >>= 2; // Scale randomness for peturbation amount
            for part in 0..K_NUM_PARTS {
        ...
        for part in 0 .. K_NUM_PARTS {
            density_thresholds[part] = !self.settings_[OutputMode::OutputModeDrums.ordinal() as usize].density[part];
        }
        ...
        for part in 0 .. K_NUM_PARTS {
            ...
            if level > density_thresholds[part] {
    ...
    fn evaluate_euclidean(&mut self) {
        ...
        for part in 0..K_NUM_PARTS {
            ...
            let fill_param_value = self.settings_[OutputMode::OutputModeEuclidean.ordinal() as usize].density[part]; // 0 - 16 from app parameter
            ...
            let mut address = (length - 1) * 32 + density_for_lut;
            if address >= LUT_RES_EUCLIDEAN_SIZE as u8 {
...
mod tests {
    ...
    fn test_initialization() {
        ...
        assert_eq!(0, generator.beat_counter_);
    ...
    fn test_evaluate_drums() {
        ...
        assert_eq!(15, generator.get_trigger_state());
    ...
    fn test_evaluate_euclidean() {
        ...
        generator.settings_[OutputMode::OutputModeEuclidean.ordinal() as usize].options = PatternModeSettings::Euclidean { chaos_amount: 0 };
        generator.settings_[OutputMode::OutputModeEuclidean.ordinal() as usize].density = [255; K_NUM_PARTS];

        for part in 0 .. K_NUM_PARTS {
            let length = 8;
            generator.set_length(part, length);
            let fill_density_param = 255;
